...
import { listVpcs } from "@/lib/huawei-cloud";

export default async function VpcDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const session = await getCurrentSession();
  const { id } = await params;
  if (!session) notFound();
  const item = (await listVpcs(session).catch(() => [])).find((vpc) => vpc.id === id);
  if (!item) notFound();
  return <NetworkDetail activeTitle="VPC" back="/services/network" facts={[["Name", item.name], ["ID", item.id], ["CIDR", item.cidr], ["Status", item.status]]} icon={<Network className="size-6" />} title={item.name} />;
}

function NetworkDetail({ activeTitle, back, facts, icon, title }: { activeTitle: string; back: string; facts: string[][]; icon: React.ReactNode; title: string }) {
  return (
